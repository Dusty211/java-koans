...
public class AboutConditionals {
    ...
    public void bitwise() {
        ...
        Counter falseCount = new Counter(false);
        String x = "Hai";
        //Couldn't find what the bitwise if condition does, but this makes it clear that 
        // it conditionally resolves the same way in the eyes of the if statement,
        // but without short circuiting like the "OR" operator.
        if (trueCount.count() | falseCount.count()) {
            x = "kthxbai";
        }
        assertEquals(x, "kthxbai");
